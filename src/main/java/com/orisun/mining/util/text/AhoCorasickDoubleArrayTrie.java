...
/**
 * 多模匹配方法：AC Automata
 * 二分trie树的基础上构建双数组trie树用于AC自动机的查询和匹配。
 *
 * @Author: Fido
 * @Since:2016-9-16
 * @Version:1.1
...
public class AhoCorasickDoubleArrayTrie<V> implements Serializable {
	...
	private static final long serialVersionUID = 429651147372194874L;
	/*
	...
	public interface IHit<V> {
		/**
		 * Hit a keyword, you can use some code like text.substring(begin, end)
		 * to get the keyword
		 * 
		 * @param begin
	...
	public interface IHitFull<V> {
		 ...
		 *            the value assigned to the keyword
		 * @param index
		 *            the index of the value assigned to the keyword, you can
		 *            use the integer as a perfect hash value
		 */
	...
	/**
	 * transition of a state, if the state is root and it failed, then returns
	 * the root
	 *
	 * @param nodePos
	 * @param c
	 ...
	 */
	public void build(Map<String, V> map) throws Exception {
		if (map == null || map.size() == 0) {
			throw new Exception("key word is empty");
		}
		new Builder().build(map);
	}

	/**
	 * Build a AhoCorasickDoubleArrayTrie from a set
	 * 
	 * @param set
	 */
	public void build(Set<String> set) throws Exception {
		if (set == null || set.size() == 0) {
			throw new Exception("key word is empty");
		}
		new Builder().build(set);
	}
	 ...
	 *
	 * @param keyChars
	 *            the char array of the key
	 * @param pos
	 *            the begin index of char array
	 * @param len
	 *            the length of the key
	 * @param nodePos
	 *            the starting position of the node for searching
	...
	/**
	 * Builder of the AhoCorasickDoubleArrayTrie
	 */
	private class Builder {
		 ...
		 */
		@SuppressWarnings({ "unchecked" })
		public void build(Set<String> set) {
			// 把值保存下来
			v = (V[]) set.toArray();
			l = new int[v.length];
			// 构建二分trie树
